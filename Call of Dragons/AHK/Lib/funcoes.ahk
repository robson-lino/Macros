...
ProcuraAteAchar(X, Y, H, W, var, img, mili) {
    ...
    {
        caminho := "imgs\" . img
        Options = r oTransRed,%var% n1
		listaCordPedras := FindClick(caminho, Option, AchouOutX, AchouOutY)
        ;ImageSearch, OutX, OutY, X, Y, H, W, *%var% *TransRed %a_scriptdir%\imgs\%img%.png
        if (AchouOutX != "")
        {
            AchouOutX := OutX
            AchouOutY := OutY
...
ProcuraAteAcharSemTrans(X, Y, H, W, var, img, mili) {
    ...
    while ((A_TickCount - Comeco) < mili)
    {
        ImageSearch, OutX, OutY, X, Y, H, W, *%var% %a_scriptdir%\imgs\%img%.png
        if (!ErrorLevel)
        {
            AchouOutX := OutX
            AchouOutY := OutY
